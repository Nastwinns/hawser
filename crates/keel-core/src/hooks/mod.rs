//! Lifecycle hooks: scripts in `.keel/hooks/` fired around keel operations.
//! Context arrives via env (`KEEL_ROOT`, `KEEL_HOOK`) and JSON on stdin.
...
pub fn fire(ws: &Workspace, hook: Hook, context: &serde_json::Value) -> Result<(), HookError> {
    ...
    let mut child = std::process::Command::new(&script)
        .current_dir(&ws.root)
        .env("KEEL_ROOT", &ws.root)
        .env("KEEL_HOOK", hook.name())
