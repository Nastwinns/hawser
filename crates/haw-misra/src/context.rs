...
//!
//! Parsing is defensive: a missing `repos` array yields an empty fleet (the
//! caller then fails open), and entries without a `path` are skipped rather
//! than rejected — the plugin degrades gracefully rather than erroring out.
...
pub struct Context {
    /// The workspace root, if the context declared one.
